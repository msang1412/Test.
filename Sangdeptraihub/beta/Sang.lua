...
-- Biến quản lý di chuyển
local isTeleporting = false
_G.CurrentTween = nil
local movementEnabled = true

-- HỆ THỐNG TWEEN MƯỢT KHÔNG GIẬT
function topos(Pos)
...
-- HÀM CHECK TELEPORT ĐƠN GIẢN
function CheckNearestTeleporter(targetPos)
    local playerPos = player.Character.HumanoidRootPart.Position
    local distanceToTarget = (targetPos.Position - playerPos).Magnitude
    
    -- Chỉ teleport khi khoảng cách đủ xa
    if distanceToTarget < 800 then
        return nil
    end
    ...
    for _, v in pairs(TableLocations) do
        local distToTeleport = (v - playerPos).Magnitude
        local distTeleportToTarget = (v - targetPos.Position).Magnitude
        
        -- Chỉ chọn teleport nếu thực sự tiết kiệm thời gian
        if distToTeleport < 200 and distTeleportToTarget < distanceToTarget * 0.7 then
            if distToTeleport < minDist then
                minDist = distToTeleport
                chosenTeleport = v
            end
        end
    end
...
function SafeRequestEntrance(teleportPos)
    -- Kiểm tra xem có đang trong quá trình di chuyển không
    ...
    if success then
        wait(1) -- Chờ ổn định sau khi teleport
        return true
    end
    return false
end

-- HÀM TELEPORT MỚI - CHỈ THỬ 2 LẦN
function StableTeleport(targetCFrame)
    if not movementEnabled or isTeleporting then return end
    ...
    -- Chỉ thử teleport nếu khoảng cách đủ xa
    if distance > 500 then
        local teleportSuccess = false
        local teleportAttempts = 0
        
        -- THỬ TELEPORT 2 LẦN
        while teleportAttempts < 2 and not teleportSuccess do
            teleportAttempts += 1
            
            local nearestTeleport = CheckNearestTeleporter(targetCFrame)
            if nearestTeleport then
                teleportSuccess = SafeRequestEntrance(nearestTeleport)
                if teleportSuccess then
                    print("Teleport thành công lần " .. teleportAttempts)
                    break
                end
            end
        ...
        -- Nếu teleport thành công, kiểm tra vị trí và kết thúc
        if teleportSuccess then
            wait(1) -- Chờ ổn định sau teleport
            isTeleporting = false
            return true
        end
    end
    
    -- NẾU TELEPORT KHÔNG THÀNH CÔNG HOẶC KHOẢNG CÁCH NGẮN -> DÙNG TWEEN
    print("Sử dụng Tween bình thường")
    local tween = topos(targetCFrame)
    if tween then
...
-- HÀM TWEEN CHÍNH SỬ DỤNG STABLETELEPORT
function Tween(targetCFrame)
    pcall(function()
        ...
        -- Sử dụng StableTeleport thay vì topos trực tiếp
        StableTeleport(FinalCFrame)
    end)
end
                    ...
                    for _, v in pairs(workspace.Enemies:GetChildren()) do
                        if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 and v.Name == Mon then
                            ...
                            repeat
                                ...
                                -- Di chuyển ổn định tới mục tiêu
                                local targetCF = CFrame.new(v.HumanoidRootPart.Position + Vector3.new(0, HeightAboveOrder, 0))
                                StableTeleport(targetCF)
...
-- CONTROL KEYS (tùy chọn - giữ phím tắt nếu muốn)
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.F then
        ...
        AutoFarmToggle:Set(_G.AutoLevel) -- Đồng bộ với toggle UI
        print("Auto Farm: " .. tostring(_G.AutoLevel))
    end
    
    if input.KeyCode == Enum.KeyCode.G then
        _G.BringEnemy = not _G.BringEnemy
        BringMobToggle:Set(_G.BringEnemy) -- Đồng bộ với toggle UI
        print("Bring Mob: " .. tostring(_G.BringEnemy))
...
print("Auto Farm Script Loaded - Complete with Fast Attack & Bring Mob")
print("Controls: F - Auto Farm, G - Bring Mob")
print("UI Toggles Available in Main Tab")

-- Toggle Auto Farm Level
local AutoFarmToggle = MainFarmTab:MakeToggle("Auto Farm Level", {
    ...
    ["Callback"] = function(Value)
        ...
        if Value then
            ...
            stopTeleport() -- Dừng di chuyển khi tắt
        end
    end
})

-- Toggle Bring Mob
local BringMobToggle = MainFarmTab:MakeToggle("Bring Mob", {
    ["Title"] = "Bring Mob",
    ["Content"] = "", 
    ["Default"] = true,
    ["Callback"] = function(Value)
        ...
        if Value then
            ...
            print("Bring Mob: Tắt")
            -- Khôi phục thuộc tính của quái khi tắt
            for _, v in pairs(workspace.Enemies:GetChildren()) do
                if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") then
                    v.HumanoidRootPart.CanCollide = true
                    v.Humanoid.WalkSpeed = 16
